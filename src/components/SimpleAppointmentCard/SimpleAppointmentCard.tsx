export default function SimpleAppointmentCard({
  ...
  `}>
    <div className="flex items-center justify-between pb-1 gap-4 lg:gap-10 md:gap-15">
      <div className="font-semibold text-sm lg:text-sm md:text-2xl truncate">{type}</div>
      <div className="text-base lg:text-lg md:text-3xl flex-shrink-0">{icon}</div>
    </div>
    <div className="flex-1">
      <div className="text-xs lg:text-xs   md:text-xl opacity-90">{time}</div>
      {doctor && (
        <div className="text-xs  md:text-lg lg:text-xs font-medium opacity-90 truncate">{doctor}</div>
      )}
    </div>
  </div>
  );
